...
class StreakFreeze {
  ...
  static Future<bool> tryApply() async {
    // Free-план: проверяем месячный лимит заморозок
    if (SubscriptionService.instance.isFree) {
      final available = SubscriptionService.instance.freezesAvailableThisMonth;
      if (available <= 0) return false;
      await SubscriptionService.instance.recordFreezeUsed();
    }
    final prefs = await SharedPreferences.getInstance();
    final count = prefs.getInt(_keyCount) ?? 0;
